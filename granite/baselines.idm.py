...
class IDMBaseline:
    ...
    def disaggregate_svi(self, tract_svi: float,
        ...
        # Handle both GeoDataFrame and DataFrame inputs
        if hasattr(prediction_locations, 'geometry'):
            # GeoDataFrame with geometry column
            coords = np.array([[geom.x, geom.y] for geom in prediction_locations.geometry])
        else:
