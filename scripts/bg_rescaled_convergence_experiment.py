"""
Block Group Constraint Convergence Experiment with Rescaled BG SVIs

Same 2x2 design as bg_convergence_experiment.py but with block group SVIs
rescaled to be consistent with the CDC tract SVI.
"""
import os
import sys
import json
...
def load_block_group_data(addresses, tract_svi, rescale=True):
    """Load block group targets and masks, optionally rescaling SVIs."""
    ...
    if rescale and block_group_targets:
        # compute pre-rescale weighted mean
    ...
    return block_group_targets, block_group_masks, bg_data, raw_targets, rescale_shift
...
def compute_correlations(runs):
    ...
    pairs = [
        ...
        ('C', 'D', 'GCN vs SAGE (tract+BG rescaled)'),
...
def format_summary(runs, correlations, rescale_shift):
    """Format the summary table."""
    lines = []
    lines.append(f"CONVERGENCE EXPERIMENT (RESCALED): Tract {TARGET_FIPS}")
    lines.append(f"BG SVI rescaling: shift={rescale_shift:+.4f} applied")
    lines.append("")
    lines.append(f"{'':30s} {'Tract-Only':>12s}    {'Tract+BG(rescaled)':>18s}")
    lines.append(f"{'':30s} {'----------':>12s}    {'------------------':>18s}")
    ...
    lines.append(f"{'GCN tract error:':30s} {val('A','tract_error'):>12s}    {val('C','tract_error'):>18s}")
    lines.append(f"{'SAGE tract error:':30s} {val('B','tract_error'):>12s}    {val('D','tract_error'):>18s}")
    lines.append(f"{'GCN BG error:':30s} {'n/a':>12s}    {val('C','bg_error'):>18s}")
    lines.append(f"{'SAGE BG error:':30s} {'n/a':>12s}    {val('D','bg_error'):>18s}")
    lines.append(f"{'GCN spatial std:':30s} {val('A','spatial_std','.4f',''):>12s}    {val('C','spatial_std','.4f',''):>18s}")
    lines.append(f"{'SAGE spatial std:':30s} {val('B','spatial_std','.4f',''):>12s}    {val('D','spatial_std','.4f',''):>18s}")
    ...
    lines.append(f"{'GCN vs SAGE r:':30s} {ab_r:>12s}    {cd_r:>18s}")
    lines.append(f"{'GCN vs SAGE rho:':30s} {ab_rho:>12s}    {cd_rho:>18s}")

    lines.append("")
    lines.append(f"Prior (unrescaled) GCN vs SAGE r:                     0.788")
    lines.append(f"Prior (unrescaled) tract error:                      ~25-28%")

    # interpretation
    lines.append("")
    constrained_r = cd.get('pearson_r', None)
    c_tract_err = runs.get('C', {}).get('tract_error', None) if runs.get('C', {}).get('success') else None
    d_tract_err = runs.get('D', {}).get('tract_error', None) if runs.get('D', {}).get('success') else None
    ...
    if constrained_r is not None and c_tract_err is not None and d_tract_err is not None:
        ...
        if avg_tract_err < 10 and constrained_r > 0.5:
            interp = "rescaling resolved constraint tension while preserving convergence benefit"
        elif avg_tract_err < 10 and constrained_r <= 0.5:
            interp = "rescaling reduced convergence; the prior agreement was an artifact of shared constraint tension"
        else:
            interp = "rescaling insufficient; additional weight tuning needed"
...
def main():
    start = time.time()
    log("Block Group Constraint Convergence Experiment (RESCALED)")
    log("=" * 60)
    ...
    # step 2: load and rescale block group data
    bg_targets, bg_masks, bg_data, raw_targets, rescale_shift = load_block_group_data(
        addresses, tract_svi, rescale=True
    )
    n_bg_used = len(bg_targets)
    n_bg_addresses = sum(m.sum() for m in bg_masks.values())
    log(f"BG targets (rescaled): {bg_targets}")
    log(f"BG targets (raw):      {raw_targets}")
    log(f"Rescale shift: {rescale_shift:+.4f}")
    log(f"Block groups: {n_bg_used}, addresses covered: {n_bg_addresses}")

    if n_bg_used < 2:
        log("STOPPING: fewer than 2 qualifying block groups.")
        return
    ...
    # C: GCN, with rescaled BG constraints
    runs['C'] = run_training('C (GCN, tract+BG rescaled)', AccessibilitySVIGNN,
                              graph_data, tract_svi, feature_names,
                              block_group_targets=bg_targets,
                              block_group_masks=bg_masks)

    # D: SAGE, with rescaled BG constraints
    runs['D'] = run_training('D (SAGE, tract+BG rescaled)', GraphSAGEAccessibilitySVIGNN,
                              graph_data, tract_svi, feature_names,
                              block_group_targets=bg_targets,
                              block_group_masks=bg_masks)
    ...
    # step 5: summary
    summary = format_summary(runs, correlations, rescale_shift)
    print("\n" + "=" * 70)
    print(summary)
    print("=" * 70)
    ...
    with open(os.path.join(RESULTS_DIR, 'bg_rescaled_convergence.json'), 'w') as f:
        json.dump(json_results, f, indent=2)
    log(f"Saved JSON to {RESULTS_DIR}/bg_rescaled_convergence.json")

    with open(os.path.join(RESULTS_DIR, 'rescaled_summary.txt'), 'w') as f:
        f.write(summary + '\n')
    log(f"Saved summary to {RESULTS_DIR}/rescaled_summary.txt")
